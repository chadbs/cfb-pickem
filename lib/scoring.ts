...
/**
 * Everyone on the same side of the same game gets the same result.
 *
 * Grading uses the game's own closing line, frozen at kickoff, not the number
 * each player happened to see when they picked. Per-pick numbers meant two
 * people could take the same team and be settled half a point apart, which on
 * a key number is the difference between a push and a loss.
 */
